...
import { getAdminBatches, createBatch, cancelBatch, restockBatch, getProduce } from '../../api/admin'
import { Plus, XCircle, ChevronDown, ChevronUp, PackagePlus } from 'lucide-react'
...
function RestockModal({ batch, onClose, onRestocked }) {
  ...
  async function handleSubmit() {
    ...
    setError(null)
    try {
      const res = await restockBatch(batch.id, qty)
      onRestocked(res.data)
      onClose()
    } catch (err) {
      setError(err.response?.data?.message ?? 'Failed to restock batch')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6 space-y-4">
        <h2 className="text-lg font-bold text-farm-text">Restock Batch</h2>
        <p className="text-sm text-gray-500">
          <span className="font-medium text-farm-text">{batch.produceName}</span>
          {' '}— currently {batch.quantityAvailable} / {batch.quantityOriginal} available
        </p>
...
function BatchRow({ batch, onCancel, onRestock }) {
    ...
    <div className="border border-gray-200 rounded-lg bg-white overflow-hidden">
      ...
      >
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0
          ${STATUS_STYLES[batch.status] ?? 'bg-gray-100 text-gray-600'}`}>
          {batch.status}
        </span>
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-5 py-4 bg-gray-50 space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {(batch.status === 'ACTIVE' || batch.status === 'DEPLETED') && (
              <button
                onClick={() => onRestock(batch)}
                className="flex items-center gap-2 bg-blue-50 hover:bg-blue-100
                  text-blue-600 text-sm font-medium px-4 py-2 rounded-md
                  transition-colors"
              >
                <PackagePlus size={15} />
                Restock
            ...
            {batch.status === 'ACTIVE' && (
              <button
                ...
                className="flex items-center gap-2 bg-red-50 hover:bg-red-100
                  text-red-600 text-sm font-medium px-4 py-2 rounded-md
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            ...
            <select name="produceId" value={form.produceId} onChange={handleChange}
              className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm
                focus:outline-none focus:ring-2 focus:ring-farm-green">
          ...
          <div>
            ...
            <input type="number" name="quantityOriginal" value={form.quantityOriginal}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" name="harvestedAt" value={form.harvestedAt}
...
export default function AdminBatches() {
  ...
  const [restockingBatch, setRestockingBatch] = useState(null)
  ...
  function handleRestocked(updatedBatch) {
    setBatches(prev => prev.map(b =>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowModal(true)}
          className="flex items-center gap-2 bg-farm-green hover:bg-farm-greenLight
      ...
      {restockingBatch && (
        <RestockModal
          batch={restockingBatch}
          onClose={() => setRestockingBatch(null)}
